...
const useFavoritesStore = create<FavoritesState>((set, get) => ({
    ...
    fetchFavorites: async () => {
        ...
        try {
            const response = await axios.get('https://guillaume-lechevallier.freeboxos.fr:5001/resources/favorites', {
    ...
    addFavorite: async (resourceId: string) => {
        ...
        try {
            const response = await axios.post(`https://guillaume-lechevallier.freeboxos.fr:5001/resources/favorite/${resourceId}`, {}, {
                withCredentials: true,
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json'
    ...
    removeFavorite: async (resourceId: string) => {
        ...
        try {
            const response = await axios.delete(`https://guillaume-lechevallier.freeboxos.fr:5001/resources/favorite/${resourceId}`, {
                withCredentials: true,
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json'
